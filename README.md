...
## Функционал
- Регистрация/логин, роли `USER`/`ADMIN`, проверка уникальности имени и логина пользователя
- CRUD используемых сущностей, валидация дат, сравнение статусов задач и подзадач, фильтр по названию задачи
- UI на Thymeleaf: страница задач пользователя, формы создания/редактирования задач, управление подзадачами и форма просмотра уведомлений пользователя
...
```
3) Базовые статусы `НОВАЯ`, `В ПРОГРЕССЕ`, `ЗАВЕРШЕНА` создаются автоматически в `Config.seedTaskStatuses`;
